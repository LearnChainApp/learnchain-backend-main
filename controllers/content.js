...
//---------------------[SETUP DO MULTER]------------------------------------------------------//
const getFileName = (uName, courseTitle, fileOriginalName) =>
  `${uName}-${courseTitle}-${fileOriginalName}`;
...
contentRouter.post(
  "/",
  [middleware.filterLoggedIn, content.array("material", 12)],
  async (req, res) => {
    if (!req.files?.length) {
      //Testa se a array files está vazia ou não existe
      res
        .status(400)
        .send({ error: "no files attached or invalid parameters" });
      return;
    }
    ...
    const fileNames = req.files.map((file) =>
      getFileName(authoruName, title, file.originalname),
    );
